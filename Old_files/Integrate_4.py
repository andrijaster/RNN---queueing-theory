...
class Maximum_principle():
    ...
    def Optimal_control(self,var_init, time_init,fun):
        ...
        def model(z,t,vreme,Lambda,mu_val,broj_mesta,g_function,Cc,c_var_pos):
            ...
            hamiltonian = [Maximum_principle.Ham_fun(i,g_function,x,p,lamb_val,mu_val,broj_mesta,Cc) for i in self.c_var_pos]
        ...
        def objective(u2_0):
            dspan = np.linspace(0, 1,1000)
            U = odeint(model, np.concatenate((var_init[:self.broj_mesta], u2_0)), dspan,
                       args = (self.vreme, self.Lambda, self.mu_val, self.broj_mesta, self.g_function,self.Cc, self.c_var_pos))
            u2 = U[-1,self.broj_mesta:]
            return u2
        
        c_varijable = []
        self.g_function = Maximum_principle.g_fun(fun,self.broj_mesta,self.price_minute)
        u2_0 = fsolve(objective, var_init[self.broj_mesta:])
        dspan = np.linspace(0, 1,1000)
        U = odeint(model, np.concatenate((var_init[:self.broj_mesta], u2_0)), dspan,
                       args = (self.vreme, self.Lambda, self.mu_val, self.broj_mesta, self.g_function,self.Cc, self.c_var_pos))
        self.solution = odeint(model, np.concatenate((var_init[:self.broj_mesta], u2_0)), dspan, args = (self.vreme, self.Lambda, self.mu_val, self.broj_mesta, self.g_function,self.Cc))
