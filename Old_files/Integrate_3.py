...
class Maximum_principle():
    ...
    def g_fun(fun,broj_mesta,price_minute):
        g = np.asarray([fun(x,price_minute) for x in range(broj_mesta)])
        return g
    
    """ Hamiltonian function """
    @staticmethod
    def Ham_fun(c_var,g,x,p_var,lamb_val,mu_val,broj_mesta,Cc):
    ...
    def __init__(self, broj_mesta, Lambda, vreme, time_control, Cc, mu_value, price_minute):
    ...
    def Optimal_control(self,var_init, time_init,fun):
        ...
        """ Model for solver """
        def model(t,z,vreme,Lambda,mu_val,broj_mesta,g_function,Cc):
            ...
            bnd = ((1,10),)
            x0 = 1
            c_var = minimize(Maximum_principle.Ham_fun,x0,method='TNC',
                             jac = Maximum_principle.jac_Ham_fun, 
                         args = (g_function,x,p,lamb_val,mu_val,broj_mesta,Cc),bounds = bnd) 
            c_var = c_var.x
            Q = Maximum_principle.trans_matrix(broj_mesta, mu_val, lamb_val,c_var)
            c_varijable.append(c_var)  
            dXdt = np.dot(Q.T,x)
        ...
        diff.set_f_params(self.vreme, self.Lambda, self.mu_val, self.broj_mesta, self.g_function,self.Cc)
        i=0
        solution = []
        c_var = []
        for i in range(len(self.time)):
            c_varijable = []
            solution.append(diff.integrate(self.time[i]))
            c_var.append(c_varijable[-1])
            i += 1
            print(i)
        self.solution = np.array(solution)
        self.c_var = np.array(c_var)
        self.x = self.solution[:,:self.broj_mesta]
    ...
    def evaluate_f_max(self,c_var,x):
        value = [self.Cc*c_var[i] + np.dot(self.g_function,x[i,:]) for i in range(len(self.time))]
        return value
...
model1 = Maximum_principle(broj_mesta, Lambda, vreme, time_control, Cc, mu_val, price_minute)
...
ax1 = plt.subplot(311)
ax1.plot(model1.time,model1.x)
ax1.set_ylim(0,1)
ax1.set_xlim(0,model1.time[-1])
ax1.set_ylabel('$P_i$')
...
ax2 = plt.subplot(312)
ax2.plot(model1.time,model1.c_var, 'black')
ax2.set_ylim(0,10)
ax2.set_xlim(0,model1.time[-1])
ax2.set_ylabel('$c(t)$')
ax2.grid()
ax2.legend('upravljanje',  loc = 'upper right')

ax3 = plt.subplot(313)
ax3.plot(model1.time,model1.x_round, lw = 5)
ax3.set_ylim(0,np.max(vrednost)+50)
ax3.set_xlim(0,model1.time[-1])
...
ax4 = plt.subplot(311)
ax4.plot(model1.time,model1.x_round)
ax4.set_ylim(0,1)
ax4.set_xlim(0,model1.time[-1])
ax4.set_ylabel('$P_i$')
...
ax5 = plt.subplot(312)
ax5.plot(model1.time,model1.c_var_round, 'black')
ax5.set_ylim(0,10)
ax5.set_xlim(0,model1.time[-1])
ax5.set_ylabel('$c(t)$')
ax5.grid()
ax5.legend('upravljanje',  loc = 'upper right')

ax6 = plt.subplot(313)
ax6.plot(model1.time,vrednost2, lw = 5)
