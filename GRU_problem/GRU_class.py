...
class GRU:
    ...
    def __init__(self, input_size=1, output_size=1, gru_size=128, num_layers=1,
                 num_steps=1, keep_prob=0.8, batch_size=64, init_learning_rate=0.5,
                 learning_rate_decay=0.99, init_epoch=5, max_epoch=100, MODEL_DIR = None, name = 'GRU_default'):
    ...
    """ BUILD GRAPH """
    def build_gru_graph_with_config(self):
        tf.reset_default_graph()
        self.gru_graph = tf.Graph()
    
        with self.gru_graph.as_default():
            ...
            def _create_one_cell():
                gru_cell = tf.contrib.rnn.GRUCell(self.gru_size)
                if self.keep_prob < 1.0:
                    gru_cell = tf.contrib.rnn.DropoutWrapper(gru_cell, output_keep_prob = self.keep_prob)
                return gru_cell
            ...
            # Before transpose, val.get_shape() = (batch_size, num_steps, gru_size)
            # After transpose, val.get_shape() = (num_steps, batch_size, gru_size)
            val = tf.transpose(val, [1, 0, 2])
    
            with tf.name_scope("output_layer"):
                # last.get_shape() = (batch_size, gru_size)
                last = tf.gather(val, int(val.get_shape()[0]) - 1, name="last_gru_output")
    
                weight = tf.Variable(tf.truncated_normal([self.gru_size, self.output_size]), name="weights")
                ...
                tf.summary.histogram("last_gru_output", last)
        ...
        return self.gru_graph


    """ TRAIN GRAPH """    
    def train_gru_graph(self, train_X, train_y, test_X, test_y):
        """
        name (str)
        gru_graph (tf.Graph)
        """
        ...
        self.graph_name = "%s_lr%.2f_lr_decay%.3f_gru%d_step%d_input%d_batch%d_epoch%d_kp%.3f_layer%d" % (
            self.name,
            self.init_learning_rate, self.learning_rate_decay,
            self.gru_size, self.num_steps,
        ...
        with tf.Session(graph = self.gru_graph) as sess:
            ...
            saver.save(sess, os.path.join(
                graph_saver_dir, "GRU_model_%s.ckpt" % self.name), global_step=epoch_step)
            
        np.savetxt('{}.csv'.format(os.path.join(
                graph_saver_dir, "GRU_%s.ckpt" % self.name)),final_prediction)
    ...
    def prediction_by_trained_graph(self, max_epoch, test_X, test_y):
        ...
        with tf.Session() as sess:
            ...
            graph_meta_path = os.path.join(
                ...
                'GRU_model_{0}.ckpt-{1}.meta'.format(self.name, max_epoch-1))
