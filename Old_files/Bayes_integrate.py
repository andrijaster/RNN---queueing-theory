...
mean_Lambda = output_ext.iloc[0,:].values.cumsum()
mean_Lambda = mean_Lambda[1:]
stand_dev = np.random.rand(mean_Lambda.shape[0]) # Predicted stddev
real_Lambda = mean_Lambda + 0.01 # Real lambda
broj_kanala_trenutno = pd.read_csv("Track_no.csv", index_col = 0)
...
_, _, _, c_var_PRESENT, _, _, _, _ = train1.train_test_fun(broj_kanala_trenutno, broj_kanala_trenutno)

c_var_PRESENT = c_var_PRESENT[0,:]
...
model4 = Maximum_principle(broj_mesta, real_Lambda, vreme, time_control, Cc, mu_val, price_minute, c_var_pos)
c_var_PRESENT, prob_PRESENT, values_PRESENT = model4.Evaluate_values_PRESENT(var_init,c_var_PRESENT,fun)
values_PRESENT_check = model4.Evaluate_values(var_init, c_var_PRESENT, fun)
...
values_PRESENT_total = np.trapz(values_PRESENT, time_control)
...
print("Present_value_real_mean_round = {}".format(values_PRESENT_total))
...
if __name__== '__main__':
    ...
    figure8 = plt.figure(figsize=(13, 9))
    ax1 = plt.subplot(311)
    ax1.plot(model1.time_control, c_var_PRESENT)
    ax1.set_xlim(0,model1.time_control[-1])
    ax1.set_ylabel('$c(t)$')
    ax1.grid()
    
    ax1 = plt.subplot(312)
    ax1.plot(model1.time_control,prob_PRESENT)
    ax1.set_ylim(0,1)
    ax1.set_xlim(0,model1.time_control[-1])
    ax1.set_ylabel('$P_i$')
    ax1.grid()
    ...
    ax1 = plt.subplot(313)
    ax1.plot(model1.time_control,values_PRESENT)
    ax1.set_xlim(0,model1.time_control[-1])
    ax1.set_ylabel('$Value$ [EUR/minute]')
    ax1.grid()
    ax1.set_xlabel('$t$ [min]')
