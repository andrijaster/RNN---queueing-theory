...
class Maximum_principle():
    ...
    def Evaluate_values_PRESENT(self, var_init, c_var, fun):
        
        def ode45_1(f, t, x0, vreme, Lambda, mu_val,broj_mesta, g_function, Cc, c_var):
            """
            4th Order Runge-Kutta method
            """
            n = len(t)
            ...
            for i in range(n-1):
                dt = t[i+1] - t[i]
                lamb_val = Maximum_principle.Lambda_value(t[i],vreme,Lambda)
                c_var1[i+1] = np.round(np.interp(t[i+1],vreme,c_var))
                Q = Maximum_principle.trans_matrix(broj_mesta, mu_val, lamb_val,c_var1[i+1])
                x[i+1] = Maximum_principle.ode45_step(f, x[i], t[i], dt, Q)                
                values[i+1] = Maximum_principle.Ham_fun(c_var1[i+1], g_function, x[i+1], lamb_val, mu_val, Cc) 
            return c_var1, x, values
        ...
        self.g_function = Maximum_principle.g_fun(fun,self.broj_mesta,self.price_minute)  
        c_var1, x, values = ode45_1(model, self.time_control, var_init, 
                       self.vreme, self.Lambda, self.mu_val, self.broj_mesta, self.g_function, self.Cc, c_var)                        
        return c_var1, x, values
...
if __name__ == "__main__":
    ...
    time_control = np.linspace(0,400,30000)
    broj_mesta = 8
    mu_val = 4.05103447
    Cc = 10
    price_minute = 5
    c_var_pos = np.arange(1,11)
    c_var_PRESENT = np.ones(81)*9
    ...
    c_var_PRESENT, prob, tot_cost = model1.Evaluate_values_PRESENT(var_init,c_var_PRESENT,fun)
