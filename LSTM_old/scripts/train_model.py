...
def train_lstm_graph(name, lstm_graph, config=DEFAULT_CONFIG):
    """
    name (str)
    lstm_graph (tf.Graph)
    """
    stock_dataset = load_data(name,
    ...
    with tf.Session(graph=lstm_graph) as sess:
        ...
        saver.save(sess, os.path.join(
            graph_saver_dir, "rnn_model_%s.ckpt" % graph_name), global_step=epoch_step)
