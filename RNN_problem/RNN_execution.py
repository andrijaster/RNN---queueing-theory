...
if __name__ == "__main__":
    ...
    for layers in steps_layers:
        ...
        for size in steps_size:
            ...
            model1 = RNN(input_size=in_size, output_size=out_size, rnn_size=size, num_layers=layers,
                             num_steps=num_unroll, keep_prob=0.8, batch_size=256, init_learning_rate=0.1,
                             learning_rate_decay=0.9, init_epoch=7, max_epoch=no_epoch, MODEL_DIR = directory, name = name1)
            model2 = RNN(input_size=in_size, output_size=out_size, rnn_size=size, num_layers=layers,
                             num_steps=num_unroll, keep_prob=1, batch_size=256, init_learning_rate=0.1,
                             learning_rate_decay=0.9, init_epoch=7, max_epoch=no_epoch, MODEL_DIR = directory, name = name2)
            
            model1.build_rnn_graph_with_config()
            model1.train_rnn_graph(train_X_mean, train_y_mean, train_X_var, train_y_var)
            prediction,loss = model1.prediction_by_trained_graph(no_epoch,test_X, test_y)
            
            model2.build_rnn_graph_with_config()
            model2.train_rnn_graph(train_X_mean, train_y_mean, train_X_var, train_y_var)
